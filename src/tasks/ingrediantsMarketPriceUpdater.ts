...
const initCachedIngredients = async (): Promise<void> => {
  cachedIngredients = await firebaseService.getDocuments<ItemDTO>(FIREBASE_COLLECTIONS.CRATE_INGREDIENT);
  isInitiated = true;
...
const ingrediantsMarketPriceUpdater = async (): Promise<void> => {
  try {
    ...
    const ingredientIds = cachedIngredients.map((ingredient) => ingredient.id);
    const searchResult = await MarketApi.getWorldMarketSearchList(MARKET_API_URLS.KR, ingredientIds.toString());
