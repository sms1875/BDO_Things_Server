...
const getChangedItems = (
    ...
    // 추가된 항목을 필터링합니다.
    const updatedItems = marketWaitList.filter(
        (item) =>
            !existingDocuments.some((doc) => doc.timestamp === item.timestamp)
    );
    // 삭제된 항목을 필터링합니다.
    const deletedItems = existingDocuments.filter(
        (doc) =>
            !marketWaitList.some((item) => item.timestamp === doc.timestamp)
    );
...
const updateMarketWaitList = async (): Promise<void> => {
    ...
    try {
        ...
        // 변경된 항목을 처리합니다.
        const { updatedItems, deletedItems } = getChangedItems(
            marketWaitList,
            cachedWaitListData
        );
