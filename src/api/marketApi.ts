...
interface IMarketApi {
  ...
  getWorldMarketList(marketUrl: string, keyType: number, mainCategory: number, subCategory?: number): Promise<MarketItemDTO[]>;
  getWorldMarketSubList(marketUrl: string, keyType: number, mainKey: number): Promise<MarketSubItemDTO[]>;
  getWorldMarketSearchList(marketUrl: string, searchResult: string): Promise<SearchedItemDTO[]>;
  getBiddingInfoList(marketUrl: string, keyType: number, mainKey: number, subKey: number): Promise<BiddingInfoDTO[]>;
  getMarketPriceInfo(marketUrl: string, keyType: number, mainKey: number, subKey: number): Promise<MarketPriceInfoDTO>;
...
class MarketApi implements IMarketApi {
  ...
  private static async fetchData(url: string, body: any): Promise<MarketApiResponseDTO> {
    try {
  ...
  private static parseDataArray<T>(responseData: MarketApiResponseDTO, parseFunction: (itemString: string) => T | null): T[] {
    const parsedItems: T[] = [];
    if (responseData.resultCode === 0 && responseData.resultMsg) {
      const itemStringArray = responseData.resultMsg.split('|');
      parsedItems.push(...itemStringArray.map(parseFunction).filter((item): item is T => item !== null));
    }
    return parsedItems;
  }
  ...
  private static parseMarketSubItem(itemString: string): MarketSubItemDTO | null {
    const itemData = itemString.split('-');
  ...
  private static parseSearchedItem(itemString: string): SearchedItemDTO | null {
    const itemData = itemString.split('-');
    if (itemData.length !== 4 || itemData.some((value) => isNaN(parseInt(value)))) {
      return null;
    }
  ...
  private static parseWaitListItem(itemString: string): WaitListItemDTO | null {
    const itemData = itemString.split('-');
    if (itemData.length !== 4 || itemData.some((value) => isNaN(parseInt(value)))) {
      return null;
    }
   ...
   */
  async getWorldMarketList(marketUrl: string, keyType: number, mainCategory: number, subCategory?: number): Promise<MarketItemDTO[]> {
    ...
    return MarketApi.parseDataArray(responseData, MarketApi.parseMarketItem);
  }
  ...
  /**
    ...
    */
  async getWorldMarketSubList(marketUrl: string, keyType: number, mainKey: number): Promise<MarketSubItemDTO[]> {
    const responseData = await MarketApi.fetchData(`${marketUrl}GetWorldMarketSubList`, { keyType, mainKey });
    return MarketApi.parseDataArray(responseData, MarketApi.parseMarketSubItem);
  }
  ...
  */
  async getWorldMarketSearchList(marketUrl: string, searchResult: string): Promise<SearchedItemDTO[]> {
    const responseData = await MarketApi.fetchData(`${marketUrl}GetWorldMarketSearchList`, { searchResult });
    return MarketApi.parseDataArray(responseData, MarketApi.parseSearchedItem);
  }
  ...
  /**
    ...
    */
  async getBiddingInfoList(marketUrl: string, keyType: number, mainKey: number, subKey: number): Promise<BiddingInfoDTO[]> {
    const responseData = await MarketApi.fetchData(`${marketUrl}GetBiddingInfoList`, { keyType, mainKey, subKey });
    return MarketApi.parseDataArray(responseData, MarketApi.parseBiddingInfo);
  }
  ...
  /**
    ...
    */
  async getMarketPriceInfo(marketUrl: string, keyType: number, mainKey: number, subKey: number): Promise<MarketPriceInfoDTO> {
  ...
  /**
     ...
     */
  async getWorldMarketWaitList(marketUrl: string): Promise<WaitListItemDTO[]> {
    const responseData = await MarketApi.fetchData(`${marketUrl}GetWorldMarketWaitList`, {});
    return MarketApi.parseDataArray(responseData, MarketApi.parseWaitListItem);
  }
}
